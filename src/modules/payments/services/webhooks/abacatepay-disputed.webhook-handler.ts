...
@injectable()
class AbacatepayDisputedWebhookHandler implements IWebhookHandlerProvider<AbacatePayTransparentWebhookRequestDTO> {
  constructor(
  ...
  public async handle(payload: AbacatePayTransparentWebhookRequestDTO): Promise<void> {
    ...
    if (
      !orderId ||
      !externalId ||
      (transparent.status !== PaymentStatus.DISPUTED && transparent.status !== PaymentStatus.UNDER_DISPUTE)
    ) {
    ...
    await this.orderRepositoryProvider.disputeOrder(orderId, payment.id);
  }
}
