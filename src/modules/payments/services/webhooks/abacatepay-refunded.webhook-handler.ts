...
@injectable()
class AbacatepayRefundedWebhookHandler implements IWebhookHandlerProvider<AbacatePayTransparentWebhookRequestDTO> {
  constructor(
  ...
  public async handle(payload: AbacatePayTransparentWebhookRequestDTO): Promise<void> {
    ...
    if (!orderId || !externalId || transparent.status !== PaymentStatus.REFUNDED) {
    ...
    await this.orderRepositoryProvider.refundPaidOrder(orderId, payment.id);
  }
}
