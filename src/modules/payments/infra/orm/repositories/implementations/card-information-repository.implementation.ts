...
class CardInformationRepository implements IRepositoryProvider<CardInformation> {
  ...
  public async find(options: CardInformationQueryOptionsDTO): Promise<CardInformation[]> {
    ...
    if (options.last4) query.andWhere('card_informations.last4 = :last4', { last4: options.last4 });
    if (options.brand) query.andWhere('card_informations.brand = :brand', { brand: options.brand });
    if (options.exp_month) query.andWhere('card_informations.exp_month = :exp_month', { exp_month: options.exp_month });
    if (options.exp_year) query.andWhere('card_informations.exp_year = :exp_year', { exp_year: options.exp_year });
    if (options.holder_name)
      query.andWhere('card_informations.holder_name = :holder_name', { holder_name: options.holder_name });
    if (options.created_at)
      query.andWhere('card_informations.created_at = :created_at', { created_at: options.created_at });
    if (options.updated_at)
      query.andWhere('card_informations.updated_at = :updated_at', { updated_at: options.updated_at });

    if (options.payment_id)
      query.andWhere('card_informations.payment_id = :payment_id', { payment_id: options.payment_id });

    if (options.limit) query.limit(options.limit);
    if (options.offset) query.offset(options.offset);
    if (options.start_date)
      query.andWhere('card_informations.created_at >= :start_date', { start_date: options.start_date });
    if (options.end_date) query.andWhere('card_informations.created_at <= :end_date', { end_date: options.end_date });
