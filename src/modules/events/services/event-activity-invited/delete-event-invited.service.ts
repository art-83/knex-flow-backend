...
class DeleteEventInvitedService {
  ...
  public async execute(user_id: string, invited_id: string) {
    ...
    const rowsDeleted = await this.eventActivityInvitedRepository.delete(invited_id);

    return { message: 'Event invited deleted successfully.', deleted: rowsDeleted };
  }
}
export { DeleteEventInvitedService };
